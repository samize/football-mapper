...
def get_hough_lines(img):
    ...
    lines = cv2.HoughLinesP(img, rho=1, theta=np.pi/180, threshold=100, minLineLength=40, maxLineGap=20)
    for line in lines:
        x1, y1, x2, y2 = line[0]
        cv2.line(blank_im, (x1, y1), (x2, y2), 255, 5)
...
def get_line_intersections(lines):
    ...
    for i in range(0, len(lines)-1):
        ...
        for j in range(i+1, len(lines)):
            ...
            if x and y \
                and x >= ((a1 < a2) * (a1-5)) + ((a2 < a1) * (a2-5)) \
                and x <= ((a1 > a2) * (a1+5)) + ((a2 > a1) * (a2+5)) \
                and x >= ((c1 < c2) * (c1-5)) + ((c2 < c1) * (c2-5)) \
                and x <= ((c1 > c2) * (c1+5)) + ((c2 > c1) * (c2+5)) \
                and y >= ((b1 < b2) * (b1-5)) + ((b2 < b1) * (b2-5)) \
                and y <= ((b1 > b2) * (b1+5)) + ((b2 > b1) * (b2+5)) \
                and y >= ((d1 < d2) * (d1-5)) + ((d2 < d1) * (d2-5)) \
                and y <= ((d1 > d2) * (d1+5)) + ((d2 > d1) * (d2+5)) \
                :
                intersections += [(x, y)]
